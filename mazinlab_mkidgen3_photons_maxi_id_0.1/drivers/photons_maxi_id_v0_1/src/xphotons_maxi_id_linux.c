...
typedef struct {
    ...
    XPhotons_maxi_id_uio_map maps[ MAX_UIO_MAPS ];
} XPhotons_maxi_id_uio_info;

/***************** Variable Definitions **************************************/
static XPhotons_maxi_id_uio_info uio_info;
...
static int uio_info_read_name(XPhotons_maxi_id_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPhotons_maxi_id_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPhotons_maxi_id_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPhotons_maxi_id_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPhotons_maxi_id_Initialize(XPhotons_maxi_id *InstancePtr, const char* InstanceName) {
	XPhotons_maxi_id_uio_info *InfoPtr = &uio_info;
