    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_IQ_DATA_0: begin
                    rdata <= int_iq[31:0];
                end
                ADDR_IQ_DATA_1: begin
                    rdata <= int_iq[63:32];
                end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_IQ_DATA_0)
            int_iq[31:0] <= (WDATA[31:0] & wmask) | (int_iq[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_IQ_DATA_1)
            int_iq[63:32] <= (WDATA[31:0] & wmask) | (int_iq[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_max_events[15:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_MAX_EVENTS_DATA_0)
            int_max_events[15:0] <= (WDATA[31:0] & wmask) | (int_max_events[15:0] & ~wmask);
    end
end
