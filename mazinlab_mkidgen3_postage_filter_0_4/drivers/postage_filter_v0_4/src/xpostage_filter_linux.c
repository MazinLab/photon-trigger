...
typedef struct {
    ...
    XPostage_filter_uio_map maps[ MAX_UIO_MAPS ];
} XPostage_filter_uio_info;

/***************** Variable Definitions **************************************/
static XPostage_filter_uio_info uio_info;
...
static int uio_info_read_name(XPostage_filter_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPostage_filter_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPostage_filter_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPostage_filter_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPostage_filter_Initialize(XPostage_filter *InstancePtr, const char* InstanceName) {
	XPostage_filter_uio_info *InfoPtr = &uio_info;
